<?php
    ...
    class QuoteStore
    {
        ...
        *******************************************************************/
        ...
        /*******************************************************************
            FUNCTION:   QuoteStore::updateLineItems
            ARGUMENTS:  $quoteId: unique id of the quote
            RETURNS:    none
            USAGE:      updates the line items and updates the price
        *******************************************************************/
        public function updateLineItems($quoteId)
        {
            $db=$this->connect();
            $query="delete from LineItem where quoteId=$quoteId";
            ...
            {
                //checks if the row is empty
                if($_POST[$desc]!="" || $_POST[$price]!="" || $_POST[$note]!="")
                {
                    //insert statement
                    $into='insert into LineItem(quoteId,description,price,secretNote)
                        values("'.$quoteId.'","'.$_POST[$desc].'","'.$_POST[$price].'","'.$_POST[$note].'")';
        ...
        /*******************************************************************
            FUNCTION:   QuoteStore::discount
            ARGUMENTS:  $quoteId: unique id of the quote
                        $type: type of discount
                        $dollar: dollar ammount
                        $percent: percent ammount
            RETURNS:    new price
            USAGE:      calculates the price and updates it
        *******************************************************************/
        public function discount($quoteId,$type,$dollar,$percent)
        {
            $db=$this->connect();

            $query="select currentPrice from Quote where quoteId=$quoteId";
            $stmt=$db->query($query);
            $row=$stmt->fetch();


            if($_POST[type]=="dollar")
                $price=$row[currentPrice]-$dollar;
            else
                $price=$row[currentPrice]-$row[currentPrice]*($_POST[percent]/100);
            $query="update Quote set currentPrice='$price' where quoteId=$quoteId";
            $db->query($query);
        ...
        /*******************************************************************
            FUNCTION:   QuoteStore::getFinalized
            ARGUMENTS:  none
            RETURNS:    a PDO statemnt contianing all finalized quotes
            USAGE:      to get a list of all the finalized quotes
        *******************************************************************/
        public function getFinalized()
        {
            $DB=$this->connect();
            $query="select quoteId,customerName from Quote where isFinalized";

            return $DB->query($query);
        }

        /*******************************************************************
            FUNCTION:   QuoteStore::saction
            ARGUMENTS:  $quoteId: unique id of the quote
            RETURNS:    none
            USAGE:      changes the quote from finalized to saction
        *******************************************************************/
        public function saction($quoteId)
        {
            $DB=$this->connect();

            $query="update Quote set isFinalized=0 where quoteId=$quoteId";
            $DB->query($query);

            $query="update Quote set isSanctioned=1 where quoteId=$quoteId";
            $DB->query($query);
        }
